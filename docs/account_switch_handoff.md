...
| 默认分支 | `main` |
| 最近已知提交 | `0c4b6e202ce466d085d53d76e934da401aff23d5` |
| 提交说明 | `feat: initialize v20 phase0 survival workspace` |
...
7. `docs\phase0_daily_log.md`
8. `docs\phase0_final_review.md`
...
Day 1 和 Day 2 都存在顶层 `app` 包名，测试必须在各自目录的独立进程中运行，不要直接从仓库根目录混跑 pytest。
...
```text
请先读取本仓库的 AGENTS.md、README.md、docs/account_switch_handoff.md、docs/migration_manifest.md，以及 day02_llm_budget/docs/day02_handoff_summary.md，作为当前上下文。继续开发时严格遵守 Phase 0 边界：不用 WebSocket、LangGraph、MCP、Redis、前端或平台化服务；Day 3 手写 ReAct，Day 4 只做最小 RAG 链路。
```

## 当前接续结论

本项目已经把 Day 1 和 Day 2 的源码、测试、文档与交接信息迁入 `v20-phase0-survival`，并推送到 GitHub。账号切换时，项目连续性由 Git 仓库、`AGENTS.md` 和 `docs` 交接文档保证，不由 Codex 账号记忆保证。
